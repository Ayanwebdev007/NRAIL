...
const OurStory = ({ onOpenBook }) => {
    ...
    const partners = [
        { name: "Orient Link Co LLC" },
        { name: "Vipa Group" },
        { name: "Lidechsi (Private) Limited" },
        { name: "Jupiter Trading Co. Ltd" },
        { name: "Regal Pulp & Paper International" },
        { name: "Roxcel Trading India Pvt Ltd" },
        { name: "Jagdamba Paper corporation" }
    ];

    // Duplicate partners for infinite loop
    const loopedPartners = [...partners, ...partners, ...partners];
    const initialScrollSet = partners.length;

    React.useEffect(() => {
        // Center the scroll initially
        if (scrollRef.current) {
            const itemWidth = 240 + 30; // box width + gap
            scrollRef.current.scrollLeft = itemWidth * initialScrollSet;
        }
    }, []);
    ...
    const handleScroll = () => {
        ...
        const itemWidth = 240 + 30;
        const totalItemsWidth = itemWidth * partners.length;

        // Use a small buffer to make the jump less noticeable
        if (scrollLeft <= 10) {
            scrollRef.current.scrollLeft = totalItemsWidth + scrollLeft;
        } else if (scrollLeft >= scrollWidth - clientWidth - 10) {
            scrollRef.current.scrollLeft = scrollLeft - totalItemsWidth;
        }
    };
    ...
    const scroll = (direction) => {
        if (scrollRef.current) {
            ...
            const scrollAmount = 240 + 30; // Scroll by one item width
        ...
        <section className="our-story" id="our-story">
            <div className="story-grid">
                ...
                <div className="story-right">
                    ...
                    >
                        <Link to="/our-story" className="view-more-btn">Read More</Link>
