...
const NewsroomPage = () => {
    ...
    // Note: Path includes "7. Newsroom" twice due to nested structure in assets
    const pdfs = import.meta.glob('../assets/7. Newsroom/7. Newsroom/**/*.pdf', { eager: true, as: 'url' });
    const excels = import.meta.glob('../assets/7. Newsroom/7. Newsroom/**/*.{xls,xlsx,xlsm}', { eager: true, as: 'url' });

    const allFiles = { ...pdfs, ...excels };

    // Extract unique years (folders) from the paths
    const years = [...new Set(Object.keys(allFiles)
        .map(path => {
            const parts = path.split('/');
            // Structure: .../7. Newsroom/7. Newsroom/[Year]/[File]
            const newsIdx = parts.lastIndexOf('7. Newsroom');
            return parts[newsIdx + 1];
        }))].sort((a, b) => b.localeCompare(a, undefined, { numeric: true }));

    const getFilesForYear = (yearName) => {
        return Object.keys(allFiles)
            .filter(path => path.includes(`/${yearName}/`))
            .map(path => ({
            ...
            <div className="pt-16 pb-12 relative overflow-hidden">
                ...
                <div className="container mx-auto px-6 md:px-24 relative z-10">
                    ...
                    >
                        {view === 'root' ? (
                            <>
                                <span className="block text-[#8b0000]">Newsroom</span>
                            </>
                        ) : (
                            <>
                                <span className="block text-black">Newsroom</span>
