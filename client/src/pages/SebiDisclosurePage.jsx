        ...
        <div className="bg-white min-h-screen font-['Outfit'] antialiased text-black selection:bg-[#8b0000] selection:text-white">
            ...
            <div className="pt-16 pb-12 relative overflow-hidden">
                ...
                <div className="container mx-auto px-6 md:px-24 relative z-10">
                    ...
                    <motion.h1
                        className="text-[40px] md:text-[48px] font-extrabold mb-8 leading-[1.1] uppercase max-w-4xl"
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.6, ease: "easeOut" }}
                    >
                        <span className="block text-black">Disclosure under Regulation 46 and 62</span>
                                ...
                                {typeof item === 'string' ? (
                                    (() => {
                                        ...
                                        return (
                                            <Component 
                                                {...extraProps}
                                                className={`group flex items-center gap-4 p-4 rounded-xl border border-black/5 bg-white transition-all ${to ? 'hover:border-[#8b0000]/30 hover:shadow-md cursor-pointer' : ''} block`}
                                            >
                                                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition-colors ${to ? 'bg-[#8b0000]/5 text-[#8b0000] group-hover:bg-[#8b0000] group-hover:text-white' : 'bg-gray-50 text-gray-400'}`}>
                                                    {(index + 1).toString().padStart(2, '0')}
                                                </div>
                                                <span className={`text-xl font-medium transition-colors flex-1 ${to ? 'text-black/80 group-hover:text-[#8b0000]' : 'text-gray-400'}`}>{item}</span>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-4 p-4 rounded-xl border border-black/5 bg-white font-bold text-xl text-black/90">
                                            ...
                                            {item.subItems.map((sub, sIdx) => {
                                                ...
                                                return (
                                                    <Link 
                                                        key={sIdx} 
                                                        to={subLink}
                                                        className="group flex items-center gap-4 p-3 rounded-lg border border-black/5 bg-white hover:border-[#8b0000]/20 hover:shadow-sm transition-all cursor-pointer block"
                                                    >
                                                        <div className="flex items-center gap-4 flex-1">
                                                            <div className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-gray-400 group-hover:bg-[#8b0000]/10 group-hover:text-[#8b0000] transition-colors text-sm">
                                                                {String.fromCharCode(97 + sIdx)})
                                                            </div>
                                                            <span className="text-lg font-medium text-black/70 group-hover:text-[#8b0000] transition-colors flex-1">{sub}</span>
