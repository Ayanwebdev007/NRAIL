...
const NrailAnnualReportsPage = () => {
    const [popupData, setPopupData] = useState(null);

    // Dynamic asset loading using Vite's glob import
    // Note: Path includes "3. NRAIL Annual Report" twice due to nested structure in assets
    const pdfs = import.meta.glob('../assets/3. NRAIL Annual Report/3. NRAIL Annual Report/**/*.pdf', { eager: true, as: 'url' });
    const excels = import.meta.glob('../assets/3. NRAIL Annual Report/3. NRAIL Annual Report/**/*.{xls,xlsx,xlsm}', { eager: true, as: 'url' });

    const allFiles = { ...pdfs, ...excels };

    // Format and sort all files (there are no subfolders for years, years are in naming)
    const annualReports = Object.keys(allFiles)
        .map(path => ({
            ...
            <div className="py-24 md:py-32 bg-[#fafafa]/50 investors-content-mobile">
                <div className="container mx-auto px-6 md:px-12 max-w-7xl">
                    <motion.div
                        ...
                        className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 py-12 investors-card-grid-mobile"
                    >
                        {annualReports.map((file, index) => (
                            <InvestorCard 
                                key={index}
                                title={file.name}
...
export default NrailAnnualReportsPage;
